...
// Post-submit summary plus a per-row list (assigned / failed), matching the
// results view on the other bulk-upload dialogs.
export default function BulkCurriculumResults({
  results,
  rows,
}: {
  results: RowResult[];
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-6 p-3 rounded-lg bg-muted/50 border border-border text-sm">
        <span className="flex items-center gap-2 text-green-600">
          <CheckCircle2 size={18} /> {ok} assigned
        </span>
                ...
                <span className="flex-1 truncate">
                  {r.success ? (
                    <span className="text-foreground/80">
                      <span className="font-mono">{course || "—"}</span> · Semester {sem || "—"} ·{" "}
